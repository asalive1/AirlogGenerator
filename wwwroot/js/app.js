...
function mapQueryType(num) {
    switch (num) {
        ...
        case 5: return "Rotator+MAID+PID";
...
async function saveSystemConfig() {
    const existing = currentSystemConfig || {};

    const cfg = {
        webPort: parseInt(document.getElementById("sys-webport").value),
        defaultServerIp: document.getElementById("server-ip").value,
        defaultServerHost: existing.defaultServerHost ?? "",
        defaultDatabaseName: existing.defaultDatabaseName ?? "",
        defaultServerUser: existing.defaultServerUser ?? "",
        defaultServerPassword: existing.defaultServerPassword ?? "",
        defaultServerPasswordSecret: existing.defaultServerPasswordSecret ?? "",
        defaultServerPasswordSecretRegion: existing.defaultServerPasswordSecretRegion ?? "",
        schedulerIntervalMinutes: parseInt(document.getElementById("sys-scheduler-interval").value),
        enableWebUi: existing.enableWebUi ?? true,
        logging: {
...
function renderSchedule(list) {
    ...
    document.querySelectorAll(".sched-execute-now").forEach(btn => {
        btn.addEventListener("click", async () => {
            ...
            const destinations = (task.destinations && task.destinations.length > 0
                ? task.destinations
                : (window.currentStation?.destinations || []))
                .filter(d => !!d);

            if (destinations.length === 0) {
                appendLog(`[ERROR] Scheduled task #${idx} has no destinations configured.`);
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    appendLog(`[SUCCESS] Scheduled task #${idx} completed for all destinations. Files written: ${data.files?.length || 0}`);
                } else {
